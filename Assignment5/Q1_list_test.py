...
class Q1Test(unittest.TestCase):
    def testEmptyCase(self):
        ...
        with self.assertRaises(ValueError):
            Q1_list.find_ordered_alphabet(empty_dictionary)

    def testKnownCases(self):
        """Tests some known cases."""
        # Test the given example
        given_dictionary = ["ART", "RAT", "CAT", "CAR"]
        self.assertTrue(Q1_list.find_ordered_alphabet(given_dictionary) in [['T', 'A', 'R', 'C'], ['A', 'T', 'R', 'C']])
        test_dictionary1 = ["baa", "abcd", "abca", "cab", "cad"]
        self.assertTrue(Q1_list.find_ordered_alphabet(test_dictionary1) in [['b', 'd', 'a', 'c'], ['d', 'b', 'a', 'c']])
        test_dictionary2 = ["ab", "bd", "c", "d"]
        self.assertEquals(Q1_list.find_ordered_alphabet(test_dictionary2), ['a', 'b', 'c', 'd'])
    ...
    def testWrongOrderCase(self):
        ...
        with self.assertRaises(ValueError):
            Q1_list.find_ordered_alphabet(wrong_dictionary)
